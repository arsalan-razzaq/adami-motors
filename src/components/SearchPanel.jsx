...
export function SearchPanel({ compact = false }) {
  ...
  const advancedFields = [
    ['engine', 'Engine', [...new Set(vehicles.map(vehicle => vehicle.engine).filter(Boolean))].sort()],
    ['transmission', 'Transmission', [...new Set(vehicles.map(vehicle => vehicle.transmission).filter(Boolean))].sort()],
    ['fuelType', 'Fuel Type', [...new Set(vehicles.map(vehicle => vehicle.fuelType).filter(Boolean))].sort()],
    ['color', 'Color', [...new Set(vehicles.map(vehicle => vehicle.color).filter(Boolean))].sort()],
    ['country', 'Available Country', [...new Set(vehicles.flatMap(vehicle => vehicle.availableCountries))].sort()],
    ['steering', 'Steering Side', ['Left', 'Right']],
  ]

  return <form onSubmit={submit} className={`glass relative z-20 rounded-xl p-5 ${compact ? '' : 'container-site -mt-14'} `}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-9">{fields.map(([key, label, options]) => <label key={key} className="text-xs font-bold uppercase text-[#666]">{label}<select value={filters[key] || ''} onChange={event => set(key, event.target.value)} className="mt-2 h-12 w-full rounded border border-[#E4E4E4] bg-white px-3 text-sm font-normal normal-case"><option value="">Any</option>{options.map(option => <option key={option}>{option}</option>)}</select></label>)}<button className="mt-auto flex h-12 items-center justify-center gap-2 rounded bg-[#D71920] px-4 font-bold uppercase text-white"><Search size={18} />Search</button><button type="button" onClick={() => setFilters({})} className="mt-auto h-12 rounded border font-bold uppercase">Reset</button></div>{more && <div className="mt-4 grid gap-3 border-t pt-4 sm:grid-cols-2 lg:grid-cols-6">{advancedFields.map(([key, label, options]) => <label key={key} className="text-xs font-bold uppercase">{label}<select value={filters[key] || ''} onChange={event => set(key, event.target.value)} className="mt-2 h-11 w-full rounded border px-2"><option value="">Any</option>{options.map(option => <option key={option}>{option}</option>)}</select></label>)}</div>}<button type="button" onClick={() => setMore(!more)} className="mt-4 flex items-center gap-2 text-xs font-bold uppercase text-[#D71920]"><SlidersHorizontal size={15} />{more ? 'Fewer Options' : 'More Options'}</button></form>
}
