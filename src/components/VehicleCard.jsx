...
export function VehicleCard({ v, list = false }) {
  ...
  return <article className={`card-hover group overflow-hidden rounded-lg border border-[#E4E4E4] bg-white ${list ? 'md:flex' : ''} ${v.status === 'sold' ? 'opacity-75' : ''}`}>
    ...
    <div className="flex flex-1 flex-col p-5"><p className="text-xs font-bold uppercase tracking-wider text-[#D71920]">{v.maker} · {v.type}</p><h3 className="mt-2 text-xl font-semibold uppercase">{v.title}</h3><div className="my-4 grid grid-cols-3 gap-2 border-y py-3 text-xs text-[#666]"><span className="flex items-center gap-1"><Calendar size={14} />{v.yearShown || v.year}</span><span className="flex items-center gap-1"><Gauge size={14} />{v.kilometersShown ? `${v.kilometersShown}${v.kilometersShown.toLowerCase() === 'n/a' ? '' : ' km'}` : km(v.mileage)}</span><span className="flex items-center gap-1"><Fuel size={14} />{v.fuelType || 'Not provided'}</span></div><div className="mt-auto flex items-center justify-between gap-3"><b className="text-xl text-[#D71920]">{money(v.price)}</b><Link className="inline-flex items-center gap-1 text-xs font-bold uppercase" to={`/vehicles/${v.carId}`}>View Details <ArrowRight size={15} /></Link></div></div>
  </article>
}
